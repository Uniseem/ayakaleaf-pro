...
import { clearSnippet, Completion, snippet, nextChar } from '@codemirror/autocomplete'
...
export const extendRequiredParameter = (
  ...
  const isWellFormedArgumentNode = argumentNode && argumentNode.getChild('OpenBrace') && argumentNode.getChild('CloseBrace')
  ...
  if (nextChar(state.doc, change.to) === '}') {
    ...
    if (isWellFormedArgumentNode) {
      ...
      const match = state.doc
        .sliceString(change.to, Math.min(state.doc.lineAt(change.from).to, argumentNode.to))
        .match(/^[^}\s,]+/)
      if (match) {
      ...
      const restOfLine = state.doc
        .sliceString(change.to, Math.min(state.doc.lineAt(change.from).to, change.from + change.insert.length))
        .split('}')[0]!

      change.to += longestCommonPrefix(change.insert.slice(change.to - change.from), restOfLine)
    }
  }
...
export const extendOverUnpairedClosingBrace = (
  ...
  if (nextChar(state.doc, change.to) === '}') {
    const unclosedBraces = countUnclosedBraces(state.doc, change.from, change.to)
    if (unclosedBraces < 0) {
      change.to++
    }
  }
...
const countUnclosedBraces = (doc: Text, from: number, to: number): number => {
  ...
  const openBraces = (textBefore.match(/\{/g) || []).length - (textBefore.match(/}/g) || []).length
