...
import type { Command } from '../../../utils/tree-operations/commands'

const commandNameFromLabel = (label: string): string | undefined => label.match(/^\\\w+/)?.[0]

export function customCommandCompletions(context: CompletionContext, commandCompletions: Completion[]) {
  const existingCommands = new Set(commandCompletions.map(item => commandNameFromLabel(item.label)).filter(Boolean))
  ...
  for (const item of items.values()) {
    if (!existingCommands.has(commandNameFromLabel(item.label)) && !item.ignoreInAutoComplete) {
      output.push({
        type: 'cmd',
        label: item.label,
...
const buildLabel = (command: Command): string => {
  return [`${command.title}`, '[]'.repeat(command.optionalArgCount ?? 0), '{}'.repeat(command.requiredArgCount ?? 0)].join('')
}
