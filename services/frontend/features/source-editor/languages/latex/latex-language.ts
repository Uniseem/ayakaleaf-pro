...
import { Tokens, commentIsOpenFold, findClosingFoldComment, getFoldRange } from '../../utils/tree-query'
...
const styleEntry = (token: string, defaultStyle: Tag): [string, Tag] => {
...
const sectionFoldRange = (node: SyntaxNode) => {
  const BACKWARDS = -1
  ...
  if (lastChild.type.is(termsModule.Whitespace)) {
    // If the sectioning command is indented on a newline
    ...
    if (sibling?.type.is(termsModule.Comment)) {
      // A trailing comment line before an indented sectioning command belongs
      // in the fold, but the sectioning command itself still gets its own
      // line. The Comment node consumes its trailing newline, so the fold
      // ends one position before the Comment does.
      return { from: content.from, to: sibling.to - 1 }
    }
    if (sibling?.type.is(termsModule.BlankLine)) {
      // Blank lines between previous content and an indented sectioning
      // command: include all but the last newline in the fold.
      return { from: content.from, to: sibling.to - 1 }
    }
  }
  if (lastChild.type.is(termsModule.BlankLine)) {
    // BlankLine can contain any number above 2 of newlines. Include every one
...
export const LaTeXLanguage = LRLanguage.define({
  ...
  parser: parser.configure({
    props: [
      foldNodeProp.add({
        Comment: (node: SyntaxNode, state: EditorState) => {
        ...
        $Environment: (node: SyntaxNode) => node.getChild('Content'),
        $Section: sectionFoldRange,
      }),
      // disable spell check in these node types when they're inside these
      // parents (empty string = any parent)
