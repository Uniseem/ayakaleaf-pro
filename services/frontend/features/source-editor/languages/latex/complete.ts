import { CompletionContext, CompletionResult, CompletionSource, ifIn } from '@codemirror/autocomplete'
...
import { customEnvironmentCompletions, findEnvironmentsInDoc } from './completions/doc-environments'
import type { Completions } from './completions/types'
...
import { ifInType, cursorIsAtBeginEnvironment, cursorIsAtEndEnvironment } from '../../utils/tree-query'
...
export function getCompletionDetails(
  ...
  const { before, existing } = groups
  const command = groups.command.toLowerCase()

  const existingKeys = existing ? splitExistingKeys(existing) : []

  const from = matchBefore.from + (before?.length || 0) + (existing?.length || 0)
  const validFor = /[^}\s]*/
...
export const makeArgumentCompletionSource = (
  ifInSpec: string[],
  builder: (builderOptions: CompletionBuilderOptions) => CompletionResult | null | Promise<CompletionResult | null>
): CompletionSource => {
  const completionSource: CompletionSource = (context: CompletionContext) => {
    ...
    const { existingKeys, from, validFor } = getCompletionDetails(match, matchBefore)
...
export const makeMultipleArgumentCompletionSource = (
  ifInSpec: string[],
  builder: (
    builderOptions: Pick<CompletionBuilderOptions, 'completions' | 'context' | 'existingKeys' | 'from' | 'validFor'>
...
export const bibKeyArgumentCompletionSource: CompletionSource = makeMultipleArgumentCompletionSource(
  ['BibKeyArgument'],
  ({ completions, context, from, validFor, existingKeys }) => {
    ...
    return {
      from,
      validFor,
      options: completions.references!.filter(item => !existingKeys.includes(item.label)),
    }
  }
)

export const refArgumentCompletionSource: CompletionSource = makeMultipleArgumentCompletionSource(
  ['RefArgument'],
  ({ completions, context, from, validFor, existingKeys }) => {
    ...
    return {
      from,
      validFor,
      options: completions.labels!.filter(item => !existingKeys.includes(item.label)),
    }
  }
)

export const packageArgumentCompletionSource: CompletionSource = makeMultipleArgumentCompletionSource(
  ['PackageArgument'],
  ({ completions, context, from, validFor, existingKeys }) => {
    ...
    return {
      from,
      validFor,
      options: completions.packages!.filter(item => !existingKeys.includes(item.label)),
    }
  }
)

export const inputArgumentCompletionSource: CompletionSource = makeArgumentCompletionSource(
...
export const includeGraphicsArgumentCompletionSource: CompletionSource = makeArgumentCompletionSource(
...
export const environmentNameCompletionSource: CompletionSource = makeArgumentCompletionSource(
  ['EnvNameGroup'],
  ({ completions, context, matchBefore, before }) => {
    if (cursorIsAtBeginEnvironment(context.state, context.pos)) {
      ...
      return {
        ...
        validFor: /^\\begin{\S*/,
        options: [...completions.commands!, ...customEnvironmentCompletions(context)],
      }
...
export const documentClassArgumentCompletionSource: CompletionSource = makeArgumentCompletionSource(
...
export const bibliographyArgumentCompletionSource: CompletionSource = makeArgumentCompletionSource(
...
export const bibliographyStyleArgumentCompletionSource: CompletionSource = makeArgumentCompletionSource(
...
const commandCompletionSource = (context: CompletionContext) => {
  ...
  return {
    from: matchBefore.to,
    options: [...completions.commands!, ...customCommandCompletions(context, completions.commands!)],
  }
}

export const inCommandCompletionSource: CompletionSource = ifInType('$CtrlSeq', context => {
...
export const beginEnvironmentCompletionSource: CompletionSource = context => {
  ...
  const beginEnv = syntaxTree(context.state).resolveInner(beginEnvToken.from, 1).parent
