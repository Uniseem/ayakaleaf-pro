...
/** Made on first use, so nothing runs in a worker until a document is open. */
let lintWorker: Worker | null = null

const getWorker = () => {
  if (!lintWorker) {
    lintWorker = new Worker(new URL('./latex-linter.worker.ts', import.meta.url), { type: 'module' })
    lintWorker.addEventListener('message', handleMessage)
  }
  return lintWorker
}
...
let linterPromise: Promise<unknown> | null = null // promise which will resolve to results of the current linting
...
const runLinter = () => {
  getWorker().postMessage({ text: currentView!.state.doc.toString() })
...
function handleMessage(event: MessageEvent) {
  if (event.data && event.data.errors && currentView) {
    ...
    // make the compile controller aware of lint errors via the editor:lint event
...
export const latexLinter = (view: EditorView) => {
  ...
  if (!linterPromise) {
    ...
    return linterPromise as Promise<Diagnostic[]>
  } else {
