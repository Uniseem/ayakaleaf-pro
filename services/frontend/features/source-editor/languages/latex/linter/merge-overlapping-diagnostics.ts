...
const diagnosticsTouchOrOverlap = (d1: Diagnostic, d2: Diagnostic) => {
  return new Range(d1.from, d1.to).touchesOrIntersects(new Range(d2.from, d2.to))
}
...
const mergeOverlappingDiagnostics = (diagnostics: Diagnostic[]) => {
  const diagnosticsByMessage = new Map<string, Diagnostic[]>()
  for (const diagnostic of diagnostics) {
    ...
    if (diagnosticsForMessage) {
      ...
      diagnosticsForMessage.sort((d1: Diagnostic, d2: Diagnostic) => d1.from - d2.from)
...
export const mergeCompatibleOverlappingDiagnostics = (diagnostics: Diagnostic[]) => {
  const allMergedDiagnostics: Diagnostic[] = []
  ...
  for (const diagnosticsForSource of groupBy(diagnostics, 'source')) {
    ...
    for (const diagnosticsForSeverity of diagnosticsBySeverity) {
      allMergedDiagnostics.push(...mergeOverlappingDiagnostics(diagnosticsForSeverity))
    }
  }
