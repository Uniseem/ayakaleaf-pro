...
import { renderMessage } from '../../../extensions/annotations'
...
// expects. Filter and adjust positions based on cursor position.
export const errorsToDiagnostics = (errors: LintError[], cursorPosition: number, docLength: number): Diagnostic[] => {
  const diagnostics: Diagnostic[] = []
  const cursorAtEndOfDocument = cursorPosition === docLength
  const suppressions: Range[] = []
  ...
  for (const error of errors) {
    ...
    const movableStart = cursorInRange && !errorAtStart && !cursorAtEndOfDocument
