...
import { Entry, EntryCommand, EntryBody, EntryType, FieldName, Comment } from '../../lezer-bibtex/bibtex.terms.mjs'
...
export const bibtexLintSource: LintSource = view => {
  ...
  // Linting can be temporarily disabled by a %%begin novalidate directive. It
  // can be re-enabled by a %%end novalidate directive
  ...
  tree.iterate({
    enter(node) {
      ...
      if (node.type.is(Comment)) {
        // Content between declarations. Can be a linter directive
...
export const checkRequiredFields = (nodeRef: SyntaxNodeRef, state: EditorState): Diagnostic[] => {
  // No errors when the tree does not hold what is being looked for
  ...
  const entryType = state.sliceDoc(entryTypeNode.from, entryTypeNode.to).toLowerCase()
  ...
  const actualFieldNames = new Set(
    actualFieldNodes
      ...
      .map(fieldNode => (fieldNode ? state.sliceDoc(fieldNode.from, fieldNode.to) : undefined))
      .filter(Boolean)
      .map(name => name?.toLowerCase())
  )

  if (actualFieldNames.has('crossref')) {
    // Key inheritance from other entries is not checked
    return []
  }
...
function createErrorMessage(missingFields: (string[] | string)[], entryTypeName: string, state: EditorState) {
  const translation =
    missingFields.length === 1 ? state.phrase('missing_field_for_entry') : state.phrase('missing_fields_for_entry')
  const or = state.phrase('or')
  const errorLines = missingFields
    .map(fieldOptions => {
      const options = Array.isArray(fieldOptions) ? fieldOptions : [fieldOptions]
...
function createOrList(fields: string[], orPhrase: string) {
  ...
  return fields.slice(0, -1).join(', ') + ` ${orPhrase} ` + fields[fields.length - 1]
}
