 ...
 *
 * It draws selection and cursor rectangles that cover the full line height,
 * which CodeMirror's own layer does not, so the highlight has no gaps between
 * lines.
 */
import { BlockInfo, BlockType, Direction, EditorView, Rect, RectangleMarker } from '@codemirror/view'
...
export function fullHeightCoordsAtPos(view: EditorView, pos: number, side?: -2 | -1 | 1 | 2 | undefined): Rect | null {
  const coords = view.coordsAtPos(pos, side as -1 | 1 | undefined)
  if (!coords) {
    return null
  }
...
function rectanglesForRange(view: EditorView, className: string, range: SelectionRange): RectangleMarker[] {
  ...
  if (visualStart && visualEnd && visualStart.from === visualEnd.from) {
    ...
    const top = visualStart ? drawForLine(range.from, null, visualStart) : drawForWidget(startBlock, false)
    const bottom = visualEnd ? drawForLine(null, range.to, visualEnd) : drawForWidget(endBlock, true)
    const between = []
  ...
  function piece(left: number, top: number, right: number, bottom: number) {
    return new RectangleMarker(className, left - base.left, top - base.top - Epsilon, right - left, bottom - top + Epsilon)
  }
  ...
  function drawForLine(from: null | number, to: null | number, line: { from: number; to: number }) {
    ...
    function addSpan(from: number, fromOpen: boolean, to: number, toOpen: boolean, dir: Direction) {
      const fromCoords = fullHeightCoordsAtPos(view, from, (from === line.to ? -2 : 2) as 2)
      const toCoords = fullHeightCoordsAtPos(view, to, (to === line.from ? 2 : -2) as 2)
