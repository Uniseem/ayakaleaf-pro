...
import { commonAncestor, matchingAncestor } from './ancestors'

export type FormattingCommand = '\\textbf' | '\\textit'
export type FormattingNodeType = string | number

export const formattingCommandMap: Record<FormattingCommand, FormattingNodeType> = {
...
export const withinFormattingCommand = (state: EditorState) => {
  ...
  return (command: FormattingCommand): boolean => {
    ...
    const isFormattedText = (range: SelectionRange): boolean => {
      ...
      const formattingCommandLeft = matchingAncestor(nodeLeft, node => node.type.is(nodeType))
      ...
      const formattingAncestor = matchingAncestor(ancestor, node => node.type.is(nodeType))
