...
import { LongArg, ShortArg, ShortTextArgument, TextArgument } from '../../lezer-latex/latex.terms.mjs'

/** The name and printed label of a `\newtheorem`-declared environment. */
export const parseTheoremArguments = (
  ...
  if (nameArgumentNode && labelArgumentNode) {
    const name = state.sliceDoc(nameArgumentNode.from, nameArgumentNode.to).trim()

    const label = state.sliceDoc(labelArgumentNode.from, labelArgumentNode.to).trim()
...
/**
 * Which style each declared theorem environment was given.
 *
 * `\theoremstyle` applies to every `\newtheorem` after it, so the document is
 * read in order and the current style carried along. Reading them out of order
 * would give a theorem the wrong style, which is the difference between
 * italic and upright body text.
 */
export const parseTheoremStyles = (state: EditorState, tree: Tree) => {
  ...
  if (topNode && topNode.name === 'LaTeX') {
    ...
    const topLevelCommands = textNode ? textNode.getChildren('Command') : topNode.getChildren('Command')
...
const argumentNodeContent = (state: EditorState, node: SyntaxNode): string | null => {
  const argumentNode = node.getChild(ShortTextArgument)?.getChild(ShortArg)

  return argumentNode ? state.sliceDoc(argumentNode.from, argumentNode.to) : null
}
