...
const getCommandName = (node: SyntaxNode, state: EditorState, childTypes: string[]): string | null => {
 ...
 * `\renewcommand`, `\newenvironment`, `\renewenvironment` and `\def` are
 * treated specially.
 */
export const enterNode = (
  ...
  if (node.type.is('NewCommand') || node.type.is('RenewCommand')) {
    ...
    const commandName = getCommandName(node.node, state, ['LiteralArgContent', 'Csname'])
    if (commandName === null) {
      return
    }
    ...
    if (optionalArguments.length > 0) {
      const argCountText = getOptionalArgumentText(state, optionalArguments[0]!)
      if (argCountText) {
        const parsed = parseInt(argCountText, 10)
        if (!Number.isNaN(parsed)) {
          argCountNumber = parsed
        }
      }
    }
    ...
    const optionalArgCount = node.node.getChildren('OptionalMacroParameter').length
    items.push({
      line: state.doc.lineAt(node.from).number,
      toLine: state.doc.lineAt(node.to).number,
      title: commandName,
    ...
    const commandArgumentsIncludingOptional = commandNode.getChildren('$Argument')
    ...
    items.push({
      ...
      requiredArgCount: commandArgumentsIncludingOptional.length - optionalArguments.length,
      type: 'usage',
      raw: undefined,
    })
  }
...
export const texOrPdfString = (state: EditorState, node: SyntaxNode, version: keyof typeof texOrPdfArgument) => {
  const commandName = getCommandName(node.node, state, ['CtrlSeq'])
  if (commandName === '\\texorpdfstring') {
    const argumentNode = node.getChildren('TextArgument')[texOrPdfArgument[version]]?.getChild('LongArg')
    if (argumentNode) {
