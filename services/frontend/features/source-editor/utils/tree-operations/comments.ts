...
export const commentIsOpenFold = (node: SyntaxNode, state: EditorState): boolean => {
  const content = state.doc.sliceString(node.from, node.to)
  return !!content.match(/%\s*\{\s*/)
}

/** Does this Comment node look like '% }' ? */
export const commentIsCloseFold = (node: SyntaxNode, state: EditorState): boolean => {
  const content = state.doc.sliceString(node.from, node.to)
  return !!content.match(/%\s*\}\s*/)
}
...
/** Given an opening fold Comment, find its closing Comment, accounting for nesting. */
export const findClosingFoldComment = (node: SyntaxNode, state: EditorState): SyntaxNode | undefined => {
...
/** The positions to fold between two fold Comments, so the fold reads `% {----}`. */
export const getFoldRange = (startNode: SyntaxNode, endNode: SyntaxNode, state: EditorState): { from: number; to: number } | null => {
