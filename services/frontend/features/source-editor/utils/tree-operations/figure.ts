...
export function centeringNodeForEnvironment(environmentNodeRef: SyntaxNodeRef): SyntaxNode | null {
  ...
  cursor.iterate(nodeRef => {
    ...
    if (nodeRef.node !== environmentNodeRef.node && nodeRef.type.is('$Environment')) {
      return false
    }
  })
