...
/**
 * Whether the cursor is close enough to an edge that something drawn beside it
 * would be half off the screen.
 */
export function isCursorNearViewportEdge(view: EditorView, pos: number) {
  return isCursorNearViewportTop(view, pos) || isCursorNearViewportBottom(view, pos)
}
