...
const cursorLayer = layer({
  ...
  markers(view) {
    ...
    for (const range of ranges) {
      ...
      if (!range.empty || !primary || canHidePrimary) {
        const className = primary ? 'cm-cursor cm-cursor-primary' : 'cm-cursor cm-cursor-secondary'

        const cursor = range.empty ? range : EditorSelection.cursor(range.head, range.head > range.anchor ? -1 : 1)
  ...
  update(update, dom) {
    ...
    return update.docChanged || update.selectionSet || updateHasMouseDownEffect(update)
  },
  mount(dom) {
...
const selectionLayer = layer({
  ...
  markers(view) {
    ...
    for (const range of view.state.selection.ranges) {
      if (!range.empty) {
        markers.push(...rectangleMarkerForRange(view, 'cm-selectionBackground', range))
      }
    }
    return markers
  },
  update(update) {
    return update.docChanged || update.selectionSet || update.viewportChanged || updateHasMouseDownEffect(update)
  },
