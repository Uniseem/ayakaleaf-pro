...
import { Annotation, Compartment, Extension, TransactionSpec } from '@codemirror/state'
import { syntaxHighlighting } from '@codemirror/language'
import { classHighlighter } from './class-highlighter'
import { FontFamily, LineHeight, userStyles } from '../utils/styles'
import { ThemeCache } from '../utils/misc'
...
export const setEditorTheme = async (editorTheme: string): Promise<TransactionSpec> => {
...
const svgUrl = (content: string) =>
  `url('data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40">${content}</svg>`)}')`
...
const createThemeFromOptions = ({
  ...
  return [
    EditorView.editorAttributes.of({
      class: activeOverallTheme === 'dark' ? 'overall-theme-dark' : 'overall-theme-light',
...
const staticTheme = EditorView.theme({
  ...
  '.cm-lint-marker-error': {
    content: svgUrl(`<circle cx="20" cy="20" r="15" fill="#f87" stroke="#f43" stroke-width="6"/>`),
  },
...
const loadSelectedTheme = async (editorTheme: string) => {
  if (!editorTheme || !(editorTheme in editorThemes)) {
    editorTheme = 'textmate' // use the default theme if unset or unknown
  }

  if (!themeCache.has(editorTheme)) {
    const { theme, highlightStyle, dark } = await editorThemes[editorTheme]!()

    // Stored in a cache, so they are reused after the first load
    const extension = [
      EditorView.theme(theme as Parameters<typeof EditorView.theme>[0], { dark }),
      EditorView.theme(highlightStyle as Parameters<typeof EditorView.theme>[0], { dark }),
    ]
