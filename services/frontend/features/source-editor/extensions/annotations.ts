...
import { Compartment, EditorState, Extension, Line, RangeSet, RangeValue, StateEffect, StateField } from '@codemirror/state'
import { syntaxTree } from '@codemirror/language'
import { debugConsole } from '@/lib/debug'

/** One entry of the compile log, placed on a line of a document. */
...
type RenderedDiagnostic = Pick<CompileLogDiagnostic, 'message' | 'severity' | 'ruleId' | 'compile' | 'source' | 'id' | 'firstOnLine'>

export type DiagnosticAction = (diagnostic: RenderedDiagnostic) => HTMLButtonElement | null

/** Buttons shown under a diagnostic. None are registered yet. */
const diagnosticActions: DiagnosticAction[] = []
 ...
 */
export const compileDiagnosticsState = StateField.define<RangeSet<CompileLogDiagnosticRangeValue>>({
  create() {
  ...
  update(value, transaction) {
    for (const effect of transaction.effects) {
      if (effect.is(setCompileDiagnosticsEffect)) {
        return RangeSet.of(
          effect.value.map(diagnostic => new CompileLogDiagnosticRangeValue(diagnostic).range(diagnostic.from, diagnostic.to)),
          true
        )
      }
    }
...
export const setAnnotations = (state: EditorState, annotations: Annotation[]) => {
...
const chooseHighlightRanges = (state: EditorState, line: Line, annotation: Annotation) => {
  const ranges: { from: number; to: number }[] = []
...
const convertAnnotationToDiagnostic = (state: EditorState, annotation: Annotation): CompileLogDiagnostic[] => {
  if (annotation.row < 0) {
...
export const renderMessage = (diagnostic: RenderedDiagnostic) => {
  ...
  const activeDiagnosticActions = diagnosticActions.map(action => action(diagnostic)).filter(Boolean) as HTMLButtonElement[]
