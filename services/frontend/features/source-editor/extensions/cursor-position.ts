import { EditorSelection, EditorState, Text, TransactionSpec } from '@codemirror/state'
 ...
 */
export const cursorPosition = ({ currentDoc: { doc_id: docId } }: { currentDoc: { doc_id: string } }) => {
  return [
...
const dispatchCursorPosition = (state: EditorState) => {
  ...
  window.dispatchEvent(new CustomEvent('cursor:editor:update', { detail: cursorPosition }))
}
...
export const restoreCursorPosition = (doc: Text, docId: string): TransactionSpec => {
  try {
    const key = buildStorageKey(docId)
    const data = customLocalStorage.getItem(key)
...
const dispatchSelectionAndScroll = (view: EditorView, from: number, to?: number) => {
  window.setTimeout(() => {
...
export const setCursorLineAndScroll = (view: EditorView, lineNumber: number, columnNumber?: number, selectText?: string) => {
