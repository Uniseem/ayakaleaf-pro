import { bracketMatching as bracketMatchingExtension, matchBrackets, type MatchResult } from '@codemirror/language'
import { Decoration, EditorView } from '@codemirror/view'
import { EditorSelection, Extension, SelectionRange, type Range } from '@codemirror/state'
  ...
  return bracketMatchingExtension({
    renderMatch: match => {
      ...
      if (matchedAdjacent(match)) {
        ...
        decorations.push(
          matchingMark.range(Math.min(match.start.from, match.end.from), Math.max(match.start.to, match.end.to))
        )
      } else {
...
const matchedAdjacent = (match: MatchResult): match is AdjacentMatchResult =>
  Boolean(match.matched && match.end && (match.start.to === match.end.from || match.end.to === match.start.from))
...
const handleDoubleClick = (evt: MouseEvent, view: EditorView) => {
  ...
  const search = (direction: Direction, position: number) => {
    ...
    if (match?.matched && match.end) {
      return EditorSelection.range(Math.min(match.start.from, match.end.from), Math.max(match.end.to, match.start.to))
    }
    return false
  }
  ...
  if (forwardsExcludingBrackets) {
    return dispatchSelection(EditorSelection.range(forwardsExcludingBrackets.from + 1, forwardsExcludingBrackets.to - 1))
  }
  ...
  if (backwardsExcludingBrackets) {
    return dispatchSelection(EditorSelection.range(backwardsExcludingBrackets.from + 1, backwardsExcludingBrackets.to - 1))
  }
