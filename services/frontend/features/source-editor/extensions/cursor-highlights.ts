import { MapMode, RangeSet, RangeValue, StateEffect, StateField, Transaction, TransactionSpec } from '@codemirror/state'
import { EditorView, hoverTooltip, layer, RectangleMarker, Tooltip } from '@codemirror/view'
...
/** Where somebody else's cursor is, and how to show it. */
        ...
        for (const highlight of effect.value) {
          if ('cursor' in highlight) {
            try {
              ...
              highlightRanges.push(new HighlightRangeValue(highlight).range(pos))
            } catch (error) {
...
const cursorTooltip = (view: EditorView, pos: number): Tooltip | null => {
  ...
  view.state.field(cursorHighlightsState).between(pos, pos, (from, to, value) => {
...
export const setCursorHighlights = (cursorHighlights: Highlight[] = []): TransactionSpec => {
  return {
...
const cursorHighlightsLayer = layer({
  ...
  update: update => {
    return (
      update.docChanged ||
      update.selectionSet ||
      update.transactions.some(tr => tr.effects.some(effect => effect.is(setCursorHighlightsEffect)))
    )
  },
  markers(view) {
    ...
    highlightRanges.between(from, to, (from, to, { highlight }) => {
      ...
      if (pos) {
        markers.push(
          new CursorMarker(highlight, 'ol-cm-cursorHighlight', pos.left - base.left, pos.top - base.top, null, pos.bottom - pos.top)
        )
      }
    })
    return markers
