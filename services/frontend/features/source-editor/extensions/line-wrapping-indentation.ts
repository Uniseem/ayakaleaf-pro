    ...
    StateField.define<number>({
      ...
      provide(field) {
        return [
          ...
          ViewPlugin.define(view => {
            const measure = {
              ...
              read(view: EditorView) {
                return (view.contentDOM.clientWidth / view.defaultCharacterWidth) * MAX_INDENT_FRACTION
              },
          ...
          ViewPlugin.define<{ decorations: DecorationSet; update: (update: ViewUpdate) => void }>(
            view => {
              ...
              const value = {
                ...
                update(update: ViewUpdate) {
                  ...
                  if (maxIndent !== previousMaxIndent) {
                    ...
                    value.decorations = updateDecorations(value.decorations, update, maxIndent)
                  }
...
export const updateDecorations = (decorations: DecorationSet, update: ViewUpdate, maxIndent: number) => {
  ...
  update.changes.iterChangedRanges((fromA, toA, fromB, toB) => {
    ...
    for (let lineNumber = fromALineNumber; lineNumber <= toALineNumber; lineNumber++) {
    ...
    for (let lineNumber = fromBLineNumber; lineNumber <= toBLineNumber; lineNumber++) {
