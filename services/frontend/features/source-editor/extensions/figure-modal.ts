import { ChangeSet, Extension, StateEffect, StateField } from '@codemirror/state'
...
import { dispatchFigureModalPasteEvent, isAllowedImageType } from '../utils/paste-image'
...
/**
 * Where the parts of one `figure` environment are in the document.
 *
 * Positions rather than text, so the modal can rewrite the caption without
 * touching the label and vice versa. They are kept up to date through `map`:
 * the modal stays open while the document is edited underneath it, including
 * by other people, and stale offsets would have it write into the wrong place.
 */
export class FigureData {
  ...
  map(changes: ChangeSet): FigureData {
    return new FigureData({
      ...
      graphicsCommandArguments: mapFromTo(this.graphicsCommandArguments, changes),
...
/** The figure the modal is editing, or null when it is inserting a new one. */
...
/**
 * Resolves once the file tree has caught up with an upload.
 *
 * A figure inserted from a new upload names a file the editor has not been
 * told about yet, and drawing it before then shows the missing-file box for a
 * moment. Waiting for the metadata to arrive avoids that -- but only for so
 * long, since the alternative to a timeout is a modal that never closes.
 */
...
/**
 * Whether two parsed figures describe the same thing.
 *
 * Compared field by field rather than by identity: parsing runs again on every
 * document change and produces a fresh object each time, so identity would
 * redraw every figure on every keystroke.
 */
export function figureDataEquals(
  a: FigureData | null,
  b: FigureData | null
): boolean {
  if (a === b) {
  ...
  return (
    ...
    a.to === b.to &&
    a.width === b.width &&
    a.unknownGraphicsArguments === b.unknownGraphicsArguments &&
    samePosition(a.caption, b.caption) &&
    samePosition(a.label, b.label) &&
    samePosition(a.graphicsCommand, b.graphicsCommand) &&
    samePosition(a.graphicsCommandArguments, b.graphicsCommandArguments) &&
    samePosition(a.file, b.file) &&
...
) => (a === b ? true : Boolean(a && b && a.from === b.from && a.to === b.to))
