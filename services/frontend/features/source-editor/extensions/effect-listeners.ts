  ...
  update(value, transaction) {
    for (const effect of transaction.effects) {
      ...
      if (effect.is(removeEffectListenerEffect)) {
        value = value.filter(
          listener => !(listener.effect === effect.value.effect && listener.callback === effect.value.callback)
        )
      }
      for (let i = 0; i < value.length; ++i) {
        ...
        if (effect.is(listener.effect)) {
          ...
          if (listener.options?.once) {
            ...
            value.splice(i, 1)
            // Keep index the same for the next iteration, since an element was removed
            --i
          }
        }
      }
...
export const removeEffectListener = <T>(view: EditorView, effect: StateEffectType<T>, callback: (value: T) => any) => {
  view.dispatch({
