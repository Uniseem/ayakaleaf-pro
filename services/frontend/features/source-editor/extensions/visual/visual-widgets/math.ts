...
/**
 * A typeset equation in place of its source.
 *
 * Rendering is asynchronous and the widget can be replaced or removed while it
 * is in flight, which is what `destroyed` and the isConnected check guard: a
 * result written into a detached element is invisible, and one written into a
 * reused element belongs to a different equation.
 *
 * The height is remembered after the first render because CodeMirror needs an
 * estimate before the widget exists, and a wrong estimate makes the scrollbar
 * jump as equations come into view.
 */
