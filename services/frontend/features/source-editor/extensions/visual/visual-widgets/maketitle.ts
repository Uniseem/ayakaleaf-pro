...
/**
 * The title block, built from what the preamble declared.
 *
 * The title and each author stay clickable and select their own source, so the
 * way to edit the title is to click the title -- not to go and find the
 * `\title` line in a preamble that is collapsed out of sight.
 */
export class MakeTitleWidget extends WidgetType {
  ...
  buildContent(view: EditorView, element: HTMLElement) {
    if (this.preamble.title) {
      const titleElement = buildTitleElement(view.state, this.preamble.title.node)
...
/**
 * One element per author, split where `\and` appeared.
 *
 * `\author{A \and B}` is a single argument in the source but two people on the
 * page, so the typeset content is walked and cut at the marker rather than
 * being shown as one run of text.
 */
