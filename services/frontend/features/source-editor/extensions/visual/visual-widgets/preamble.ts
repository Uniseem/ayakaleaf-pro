...
/**
 * The bar that stands in for a collapsed preamble.
 *
 * The preamble is configuration rather than writing, and in the visual editor
 * it is folded away by default -- so this is both the notice that something is
 * hidden and the way back to it. Expanding puts the cursor at position 0
 * instead of dispatching an effect, because the preamble stays open exactly as
 * long as the cursor is inside it.
 */
export class PreambleWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    helpLink.href = '/learn/latex/Learn_LaTeX_in_30_minutes#The_preamble_of_a_document'
