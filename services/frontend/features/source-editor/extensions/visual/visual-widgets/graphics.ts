...
import { FigureData, figureDataEquals } from '../../figure-modal'
...
/**
 * The image a figure includes, drawn in place of the environment.
 *
 * Three kinds of source need three different treatments: a PDF is rendered
 * through pdf.js onto a canvas, an SVG is fetched and re-served through a blob
 * URL, and everything else is an ordinary img.
 *
 * When the file cannot be found the path is shown instead. A missing graphic
 * is nearly always a typo or a file not yet uploaded, and the path is what
 * tells the writer which of the two it is.
 */
export class GraphicsWidget extends WidgetType {
  ...
  eq(widget: GraphicsWidget) {
    return (
      ...
      figureDataEquals(this.figureData, widget.figureData)
    )
  }
