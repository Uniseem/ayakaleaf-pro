...
import { BlockInfo, EditorView, lineNumbers as _lineNumbers } from '@codemirror/view'
import { throttle, type Debounced } from '@/lib/timing'
...
export function lineNumbers(): Extension {
  let listener: Debounced<(event: MouseEvent) => boolean> | null
  ...
  function selection(view: EditorView, start: BlockInfo, end: BlockInfo) {
    const clamp = (num: number) => Math.max(0, Math.min(view.state.doc.length, num))
  ...
  return _lineNumbers({
    domEventHandlers: {
      mousedown: (view, line, event) => {
        ...
        // If there is already a listener, disable it
        disableListener()
        view.dispatch({
        ...
        listener = throttle((event: MouseEvent) => {
          // Check for a missed mouseup event by validating that the
