...
import { getIndentUnit, IndentContext, indentString, syntaxTree } from '@codemirror/language'
...
const wrapRangeInList = (state: EditorState, range: SelectionRange, environment: string, prefix = '') => {
...
const wrapRangesInList =
  ...
  (view: EditorView): boolean => {
    view.dispatch(
      view.state.changeByRange(range => wrapRangeInList(view.state, range, environment)),
      { scrollIntoView: true }
    )
    return true
  }

const unwrapRangeFromList = (state: EditorState, range: SelectionRange, environment: string) => {
...
const unwrapRangesFromList =
  ...
  (view: EditorView): boolean => {
    view.dispatch(
      view.state.changeByRange(range => unwrapRangeFromList(view.state, range, environment)),
      { scrollIntoView: true }
    )
    return true
  }

const toggleListForRange = (view: EditorView, range: SelectionRange, environment: string) => {
  const ancestorNode =
    ancestorNodeOfType(view.state, range.head, ListEnvironment) ?? wrappedNodeOfType(view.state, range, ListEnvironment)
  ...
  if (ancestorNode) {
    ...
    if (beginEnvNode && endEnvNode) {
      const beginEnvNameNode = beginEnvNode?.getChild('EnvNameGroup')?.getChild('ListEnvName')

      const endEnvNameNode = endEnvNode?.getChild('EnvNameGroup')?.getChild('ListEnvName')

      if (beginEnvNameNode && endEnvNameNode) {
        const envName = view.state.sliceDoc(beginEnvNameNode.from, beginEnvNameNode.to).trim()
        ...
        if (envName === environment) {
          ...
          const changeSpec: ChangeSpec[] = [
            {
              ...
              to: emptyBeginLine ? Math.min(beginLine.to + 1, view.state.doc.length) : beginEnvNode.to,
              insert: '',
            },
            {
              from: emptyEndLine ? Math.max(endLine.from - 1, 0) : endEnvNode.from,
          ...
          const itemNodes = descendantsOfNodeWithType(ancestorNode, 'Item', ListEnvironment)
          ...
          if (itemNodes.length > 0) {
            ...
            for (const itemNode of itemNodes) {
              ...
              const lineBeforeCommand = view.state.sliceDoc(line.from, itemNode.from)
...
export const toggleListForRanges = (environment: string) => (view: EditorView) => {
  view.dispatch(
    view.state.changeByRange(range => toggleListForRange(view, range, environment)),
    { scrollIntoView: true }
  )
  return true
}
