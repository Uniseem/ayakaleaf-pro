...
export const findCurrentSectionHeadingLevel = (state: EditorState) => {
  const selections = state.selection.ranges.map(range => rangeInfo(state, range))
...
export const rangeInfo = (state: EditorState, range: SelectionRange): RangeInfo => {
  ...
  view.dispatch(
    view.state.changeByRange(range => {
      ...
      if (level === 'text' && info.command) {
        ...
        if (argument) {
          const content = view.state.sliceDoc(argument.from + 1, argument.to - 1)
          // map through the prefix only
          const changedRange = range.map(
            view.state.changes([{ from: info.command.from, to: argument.from + 1, insert: '' }]),
            1
          )
          return {
            range: changedRange,
        ...
        // map through the prefix only
        const changedRange = originalRange.map(view.state.changes([{ from: range.from, insert: `${insert.prefix}` }]), 1)
