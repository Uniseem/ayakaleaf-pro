...
import { Compartment, EditorSelection, Prec, TransactionSpec } from '@codemirror/state'
...
type LooseVim = {
  ...
  defineMotion: (name: string, fn: (cm: CodeMirror, head: { line: number; ch: number }, motionArgs: Record<string, unknown>) => unknown) => void
  mapCommand: (keys: string, type?: string, name?: string, args?: unknown, extra?: Record<string, unknown>) => void
  exitInsertMode: (cm: unknown) => void
}

const customiseVimOnce = (vim: typeof Vim, _CodeMirror: typeof CodeMirror) => {
  const _Vim = vim as unknown as LooseVim
  if (customisedVim) {
    return
  }
  ...
  _Vim.defineAction('insertModeCtrlC', (cm: CodeMirror) => {
    if (hasNonEmptySelection(cm)) {
      ...
      _Vim.exitInsertMode(cm as Parameters<typeof _Vim.exitInsertMode>[0])
    }
  })
  ...
  _Vim.defineMotion(
    'moveByCharacters',
    function (cm: CodeMirror, head: { line: number; ch: number }, motionArgs: Record<string, unknown>) {
      const { cm6: view } = cm
      ...
      return new _CodeMirror.Pos(finishLine.number - 1, cursor.head - finishLine.from)
    }
  )
  ...
  _EmacsHandler.addCommands({
    openSearch(handler: EmacsHandler) {
      ...
      if (emacsSearchCloseListener) {
        document.removeEventListener('cm:emacs-close-search-panel', emacsSearchCloseListener)
      }
      emacsSearchCloseListener = () => {
        jumpToLastMark(handler)
      }
      document.addEventListener('cm:emacs-close-search-panel', emacsSearchCloseListener)
    },
    save() {
      window.dispatchEvent(new Event('pdf:recompile'))
    },
...
export const setKeybindings = async (selectedKeybindings = 'default'): Promise<TransactionSpec> => {
  if (selectedKeybindings === 'none') {
    selectedKeybindings = 'default'
  }

  const selectedOption = options.find(option => option.name === selectedKeybindings)
