...
export const autoComplete = ({ enabled }: AutoCompleteOptions) => autoCompleteConf.of(createAutoComplete({ enabled }))

export const setAutoComplete = ({ enabled }: AutoCompleteOptions): TransactionSpec => {
  return {
    effects: autoCompleteConf.reconfigure(createAutoComplete({ enabled })),
  }
}

const createAutoComplete = ({ enabled }: AutoCompleteOptions) => {
  if (!enabled) {
    return []
  }
...
const autocompleteTheme = EditorView.baseTheme({
  ...
  '.cm-tooltip .cm-completionDetail': {
    ...
    // This element is used for the symbol palette, so it takes the
    // symbol palette font
