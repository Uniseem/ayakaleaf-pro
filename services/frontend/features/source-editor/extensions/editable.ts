...
const readOnlyAttributes = [EditorState.readOnly.of(true), EditorView.editable.of(false), focusableReadOnly, hideCursor]

const editableAttributes = [EditorState.readOnly.of(false), EditorView.editable.of(true)]
 ...
 * tabindex="0" is added in read-only mode to ensure the editor remains focusable for keyboard shortcuts.
 */
...
export const setEditable = (value = true): TransactionSpec => {
  return {
    effects: [readOnlyConf.reconfigure(value ? editableAttributes : readOnlyAttributes)],
  }
}
