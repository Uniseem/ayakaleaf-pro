...
import { getNormalTextSpansFromLine } from '../../utils/tree-operations/text'
...
export class Word {
  ...
  constructor(options: {
    ...
    if (
      ...
      throw new Error('Spellcheck: invalid word')
    }
    this.text = text
    this.from = from
    this.to = to
...
export const buildSpellCheckResult = (
  ...
  // The indexes come back from the worker, which was given this same list --
  // so every one of them is in range.
