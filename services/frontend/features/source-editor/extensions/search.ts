...
import { beforeChangeDocEffect } from './before-change-doc'
...
export const getStoredSelection = (state: EditorState) => state.field(storedSelectionState)

export const setStoredSelection = (selection: EditorSelection | null) => {
  return {
    effects: [
      storedSelectionEffect.of(selection),
      highlightSelectionMatchesConf.reconfigure(selection ? [] : highlightSelectionMatchesExtension),
    ],
  }
}
...
const scrollToMatch = (range: SelectionRange, view: EditorView) => {
  ...
  const strategy =
    (coords.from && coords.from.top < scrollRect.top) || (coords.to && coords.to.bottom > scrollRect.bottom) ? 'center' : 'nearest'
      ...
      createPanel: () => {
        ...
        return {
          ...
          mount() {
            ...
            const searchInput = dom.querySelector<HTMLInputElement>('[main-field]')
            if (searchInput) {
              searchInput.focus()
              searchInput.select()
            }
      ...
      for (const tr of update.transactions) {
        ...
        for (const effect of tr.effects) {
          if (effect.is(setSearchQuery)) {
            ...
            // the Vim extension, so bail out here in Vim mode. The Vim
            // extension sticks an extra property on the query value that
            // can be checked
            ...
            if (next) {
              ...
              if (searchPanelOpen(tr.startState)) {
                spec.effects = scrollToMatch(EditorSelection.range(next.from, next.to), update.view)
              }
...
const searchFormTheme = EditorView.theme({
  '.ol-cm-search-form': {
    ...
    '--ol-cm-search-form-focus-shadow': 'inset 0 1px 1px rgb(0 0 0 / 8%), 0 0 8px rgb(102 175 233 / 60%)',
    '--ol-cm-search-form-error-shadow': 'inset 0 1px 1px rgb(0 0 0 / 8%), 0 0 8px var(--red-50)',
