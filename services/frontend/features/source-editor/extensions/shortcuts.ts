...
import { deleteToVisualLineEnd, deleteToVisualLineStart } from './visual-line-selection'
...
const addNewCommentFromKbdShortcut = () => {
  window.dispatchEvent(new CustomEvent('add-new-review-comment', { detail: { location: 'shortcut' } }))
  return true
}
...
export const shortcuts = Prec.high(
  keymap.of([
    ...
    // which uses a US keyboard layout, so more mappings are needed.
    ...
    // Note: both Ctrl and Command versions need to work on macOS, for backwards compatibility,
