...
import { newThreadId as uniqueId } from '@/lib/comments'
...
export class HunspellManager {
  ...
  constructor(
    ...
    // Both are static files this client ships, put in place by
    // scripts/copy-hunspell.mjs; the original reads them from page metadata
    // because its server decides where they live.
    this.baseAssetPath = new URL('/', window.location.href).toString()

    this.dictionariesRoot = '/dictionaries/'
  ...
  receive(event: MessageEvent) {
    ...
    if (id) {
      ...
      debugConsole.error('Spell check loading failed', rest.loadingFailed)
