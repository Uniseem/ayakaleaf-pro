...
import { useCodeMirrorStateContext, useCodeMirrorViewContext } from '../components/codemirror-context'
import { FigureModalSource } from '../components/toolbar/insert-figure-dropdown'
import * as commands from '../extensions/toolbar/commands'
import { setSectionHeadingLevel } from '../extensions/toolbar/sections'
...
/** The Edit, Insert and Format menus, filled in while the editor is showing. */
export const useToolbarMenuBarEditorCommands = () => {
  const view = useCodeMirrorViewContext()
  const state = useCodeMirrorStateContext()
  const { t } = useTranslation()
  const { view: layoutView } = useLayout()
  const editorIsVisible = layoutView === 'editor'
  const { canWrite, canReview } = useProject()
  const trackedWrite = canWrite || canReview
  const comment = canWrite || canReview
  ...
  const { hasLinkUrlFeature, symbolPaletteAvailable } = useSite()
  ...
  }, [t, editorIsVisible, openFigureModal, trackedWrite, isTeXFile, hasLinkUrlFeature])
  ...
  }, [symbolPaletteAvailable, t, toggleSymbolPalette, editorIsVisible, isTeXFile, trackedWrite])
}
