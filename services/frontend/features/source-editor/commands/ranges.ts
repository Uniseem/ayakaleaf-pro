...
import { ensureSyntaxTree, foldedRanges, foldEffect, syntaxTree } from '@codemirror/language'
...
import { formattingCommandMap, type FormattingCommand, type FormattingNodeType } from '../utils/tree-operations/formatting'

export const wrapRanges =
  (prefix: string, suffix: string, wrapWholeLine = false, selection?: (range: SelectionRange, view: EditorView) => SelectionRange) =>
  (view: EditorView): boolean => {
    ...
    view.dispatch(
      view.state.changeByRange(range => {
        ...
        const changedRange = range.map(view.state.changes([{ from: range.from, insert: `${insert.prefix}` }]), 1)
  ...
  view.dispatch(
    view.state.changeByRange(range => {
      ...
      if (range.empty) {
        ...
        // Calculate line start/end including folded ranges. Each pass can pull
        // in more folded ranges when several folds share one editor line, so
        // the selection is widened until it settles.
        let changed
        do {
          changed = false
          foldedRangesInDocument.between(lineStart, lineEnd, (from, to) => {
            const newLineStart = Math.min(view.state.doc.lineAt(from).from, lineStart)
        ...
        return {
          ...
          changes: [
            {
              from: lineEnd,
              insert: view.state.lineBreak + view.state.doc.slice(lineStart, lineEnd),
            },
          ],
        ...
        if (range.head > range.anchor) {
          // Duplicating to the right, so the selected range moves too
          newSelectionRange = EditorSelection.range(range.head, range.head + (range.to - range.from))
        }
        return {
...
const skipParentNodeTypes = ['LongArg', 'TextArgument', 'OpenBrace', 'CloseBrace']

function getParentNode(position: number | SyntaxNode, state: EditorState, assoc: 0 | 1 | -1 = 1): SyntaxNode | undefined {
...
function wrapRangeInCommand(state: EditorState, range: SelectionRange, command: string) {
  ...
  return {
    changes,
    range: moveRange(range, range.from + command.length + 1, range.from + command.length + content.length + 1),
  }
}

function moveRange(range: SelectionRange, newFrom: number, newTo: number) {
  const forwards = range.from === range.anchor
  return forwards ? EditorSelection.range(newFrom, newTo) : EditorSelection.range(newTo, newFrom)
}
...
function getWrappingAncestor(node: SyntaxNode, nodeType: FormattingNodeType): SyntaxNode | null {
  for (let ancestor: SyntaxNode | null = node; ancestor; ancestor = ancestor.parent) {
    ...
    if (formattingCommandMap[ancestor.type.name as FormattingCommand]) {
      // Possibly multiple levels deep in bold/non-bold, so give up here
      return null
    }
  }
  return null
...
function adjustRangeIfNeeded(nodeType: FormattingNodeType, range: SelectionRange, state: EditorState) {
  ...
  if (parent?.type.is('$ToggleTextFormattingCommand') && spansWholeArgument(parent, range)) {
    return bubbleUpRange(nodeType, ancestorOfNodeWithType(nodeLeft, '$ToggleTextFormattingCommand'), range)
  }

  if (!parentLeft) {
    // Not trying to unbold, so no adjustment of the range
    return bubbleUpRange(nodeType, ancestorOfNodeWithType(nodeLeft, '$ToggleTextFormattingCommand'), range)
  }
  ...
  // parentLeft is the $ToggleTextFormattingCommand, so now check whether the
  // range ends to the right of the closing brace. (parent is TextArgument,
  // grandparent is $ToggleTextFormattingCommand)
...
function spansWholeArgument(commandNode: SyntaxNode | null, range: SelectionRange): boolean {
  const argument = commandNode?.getChild('TextArgument')?.getChild('LongArg')
  return Boolean(argument && argument.from === range.from && argument.to === range.to)
}

function bubbleUpRange(nodeType: string | number, node: SyntaxNode | null, range: SelectionRange) {
  let currentRange = range
  for (
    let ancestorCommand: SyntaxNode | null = ancestorOfNodeWithType(node, '$ToggleTextFormattingCommand');
    spansWholeArgument(ancestorCommand, currentRange);
    ancestorCommand = ancestorOfNodeWithType(ancestorCommand!.parent, '$ToggleTextFormattingCommand')
  ) {
    if (!ancestorCommand) {
      break
    }
    currentRange = moveRange(currentRange, ancestorCommand.from, ancestorCommand.to)
    if (ancestorCommand.type.is(nodeType)) {
      const argumentNode = ancestorCommand.getChild('TextArgument')?.getChild('LongArg')
      if (!argumentNode) {
        return range
      }
...
export function toggleRanges(command: FormattingCommand) {
  ...
  /* The situations handled here, with the selection marked within <>:
   *
   * 1. If the parent node at start and end of selection is different -> do
   *    nothing. Case 8 is an exception to this.
   * 2. If selection contains a BlankLine (i.e. two newlines in a row) -> do
   *    nothing (\textbf doesn't allow paragraph breaks).
   ...
   *    two: \textbf{this <is} a \textbf{te>st} → \textbf{this <is a te>st}
   * 9. If the selection spans the end of a \textbf, into the parent of the
   *    command, then extend the \textbf: \textbf{this <is} a test> → \textbf{this <is a test>}
  ...
  return (view: EditorView): boolean => {
    ...
    view.dispatch(
      view.state.changeByRange(initialRange => {
        ...
        const ancestorAtStartOfRange = getParentNode(range.from, view.state, range.from === 0 ? 1 : -1)
        const ancestorAtEndOfRange = range.empty
          ? ancestorAtStartOfRange
          : getParentNode(range.to, view.state, range.to === view.state.doc.length ? -1 : 1)
        ...
        if (!isSingleNodeSelected && ancestorAtStartOfRange !== ancestorAtEndOfRange) {
          // But handle the exception of case 8
          const ancestorAtStartIsWrappingCommand = ancestorAtStartOfRange?.type.is(nodeType)

          const ancestorAtEndIsWrappingCommand = ancestorAtEndOfRange && ancestorAtEndOfRange.type.is(nodeType)
          ...
          if (
            ...
            // Test for case 8
            const nextAncestorAtStartOfRange = ancestorAtStartOfRange.parent.parent.parent
            const nextAncestorAtEndOfRange = ancestorAtEndOfRange.parent.parent.parent

            if (nextAncestorAtStartOfRange === nextAncestorAtEndOfRange) {
              // Join the two ranges
              const textBetweenRanges = view.state.sliceDoc(ancestorAtStartOfRange.to, ancestorAtEndOfRange.from)
              const ancestorStartArgumentNode = ancestorAtStartOfRange.lastChild?.getChild('LongArg')
              const ancestorEndArgumentNode = ancestorAtEndOfRange.lastChild?.getChild('LongArg')
              if (!ancestorStartArgumentNode || !ancestorEndArgumentNode) {
                throw new Error("Can't find argument node")
              }
              const actualContent = view.state.sliceDoc(ancestorAtStartOfRange.from, ancestorAtEndOfRange.to)
              const firstCommandArgument = view.state.sliceDoc(ancestorStartArgumentNode.from, ancestorStartArgumentNode.to)
              const secondCommandArgument = view.state.sliceDoc(ancestorEndArgumentNode.from, ancestorEndArgumentNode.to)
              validateReplacement(
              ...
              return {
                changes,
                range: moveRange(range, range.from, range.to - command.length - 1 - 1),
              }
            }
          }

          if (ancestorAtEndIsWrappingCommand && ancestorAtEndOfRange.parent?.parent?.parent === ancestorAtStartOfRange) {
            // Extend to the left. Case 10
            const contentUpToCommand = view.state.sliceDoc(range.from, ancestorAtEndOfRange.from)
            const ancestorEndArgumentNode = ancestorAtEndOfRange.lastChild?.getChild('LongArg')
            if (!ancestorEndArgumentNode) {
              throw new Error("Can't find argument node")
            }
            const commandContent = view.state.sliceDoc(ancestorEndArgumentNode.from, ancestorEndArgumentNode.to)
            const actualContent = view.state.sliceDoc(range.from, ancestorAtEndOfRange.to)
            validateReplacement(`${contentUpToCommand}${command}{${commandContent}}`, actualContent)
            const changes = view.state.changes([
              {
                from: range.from,
                to: ancestorAtEndOfRange.to,
            ...
            return {
              changes,
              range: moveRange(range, range.from + command.length + 1, range.to),
            }
          }
          ...
          if (
            ancestorAtStartIsWrappingCommand &&
            ancestorAtStartOfRange &&
            ancestorAtStartOfRange.parent?.parent?.parent === ancestorAtEndOfRange
          ) {
            // Extend to the right. Case 9
            const contentAfterCommand = view.state.sliceDoc(ancestorAtStartOfRange.to, range.to)
            const ancestorStartArgumentNode = ancestorAtStartOfRange.lastChild?.getChild('LongArg')
            if (!ancestorStartArgumentNode) {
              throw new Error("Can't find argument node")
            }
            const commandContent = view.state.sliceDoc(ancestorStartArgumentNode.from, ancestorStartArgumentNode.to)
            const actualContent = view.state.sliceDoc(ancestorAtStartOfRange.from, range.to)
            validateReplacement(`${command}{${commandContent}}${contentAfterCommand}`, actualContent)
            const changes = view.state.changes([
              {
                from: ancestorAtStartOfRange.from,
                to: range.to,
        ...
        const isCursorBeforeAncestor = range.empty && ancestor && range.from === ancestor.from && ancestor.type.is(nodeType)

        // With no ancestor node, or a parent that is not an existing \textbf,
        // just wrap it in a range. Case 3.
        ...
        // Trim from the beginning. Case 4
        if (range.from === argumentNode.from && range.to !== argumentNode.to) {
          const textAfterSelection = view.state.sliceDoc(range.to, argumentNode.to)
          const wholeCommand = view.state.sliceDoc(ancestor.from, ancestor.to)
          validateReplacement(`${command}{${content}${textAfterSelection}}`, wholeCommand)
          const changes = view.state.changes([
            {
              from: ancestor.from,
              to: ancestor.to,
          ...
          return {
            range: moveRange(range, range.from - command.length - 1, range.to - command.length - 1),
            changes,
          }
        }

        // Trim from the end. Case 5
        if (range.to === argumentNode.to && range.from !== argumentNode.from) {
          const textBeforeSelection = view.state.sliceDoc(ancestor.from, range.from)
          const wholeCommand = view.state.sliceDoc(ancestor.from, ancestor.to)
          ...
          // Shift the selection forward by the inserted }
          return {
        ...
        // Split the command in two. Case 6
        if (range.from !== argumentNode.from && range.to !== argumentNode.to) {
          const textBeforeSelection = view.state.sliceDoc(ancestor.from, range.from)
          const textAfterSelection = view.state.sliceDoc(range.to, ancestor.to)
          const wholeCommand = view.state.sliceDoc(ancestor.from, ancestor.to)
          validateReplacement(`${textBeforeSelection}${content}${textAfterSelection}`, wholeCommand)
          const changes = view.state.changes([
            {
              from: ancestor.from,
              to: ancestor.to,
        ...
        if (spansWholeArgument(ancestor, range)) {
          const argumentContent = view.state.sliceDoc(argumentNode.from, argumentNode.to)
          const wholeCommand = view.state.sliceDoc(ancestor.from, ancestor.to)
          validateReplacement(`${command}{${content}}`, wholeCommand)
          const changes = view.state.changes([{ from: ancestor.from, to: ancestor.to, insert: argumentContent }])
