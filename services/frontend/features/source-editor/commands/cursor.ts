...
import { emitShortcutEvent } from '../extensions/toolbar/utils/analytics'
...
export const cloneSelectionVertically =
  ...
  view => {
    ...
    filteredRanges = filteredRanges.filter(item => item.from !== nextRange.from && item.to !== nextRange.to)
    const selection = EditorSelection.create(filteredRanges.concat(nextRange), filteredRanges.length)
