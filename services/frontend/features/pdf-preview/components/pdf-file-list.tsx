...
import type { PdfFileData, PdfFileDataList } from '../util/types'
    ...
    <>
      ...
      {fileList.top.map(file => (
        <li key={file.path} role="menuitem">
          <DropdownItem role="link" download={basename(file)} href={file.downloadURL || file.url}>
            {file.path}
          </DropdownItem>
        </li>
      ))}

      {fileList.other.length > 0 && fileList.top.length > 0 && <DropdownDivider />}
      ...
      {fileList.archive?.fileCount !== undefined && fileList.archive?.fileCount > 0 && (
        <li role="menuitem">
          <DropdownItem role="link" download={basename(fileList.archive)} href={fileList.archive.url}>
