...
/** The zoom percentage, which opens a menu of zoom levels and a box to type one. */
function PdfZoomDropdown({ requestPresentationMode, setZoom, rawScale }: PdfZoomDropdownProps) {
  const { t } = useTranslation()

  const shortcuts = isMac() ? macShortcuts : otherShortcuts

  const [customZoomValue, setCustomZoomValue] = useState<string>(rawScaleToPercentage(rawScale))

  useEffect(() => {
    setCustomZoomValue(rawScaleToPercentage(rawScale))
  }, [rawScale])

  const showPresentOption = typeof document !== 'undefined' && document.fullscreenEnabled

  return (
    <Dropdown
      onSelect={eventKey => {
        if (eventKey === 'custom-zoom' || eventKey === undefined) {
          return
        }
    ...
    >
      ...
      <DropdownMenu className="pdfjs-zoom-dropdown-menu">
        <li role="none">
          <DropdownItem disabled as="div" className="pdfjs-custom-zoom-menu-item" eventKey="custom-zoom">
            <FormControl
        ...
        <DropdownDivider />
        <li role="none">
          <DropdownItem as="button" eventKey="zoom-in" trailingIcon={<Shortcut keys={shortcuts['zoom-in']} />}>
            {t('zoom_in')}
          </DropdownItem>
        </li>
        <li role="none">
          <DropdownItem as="button" eventKey="zoom-out" trailingIcon={<Shortcut keys={shortcuts['zoom-out']} />}>
            {t('zoom_out')}
          </DropdownItem>
        </li>
        <li role="none">
          <DropdownItem as="button" eventKey="page-width" trailingIcon={<Shortcut keys={shortcuts['fit-to-width']} />}>
            {t('fit_to_width')}
          </DropdownItem>
        </li>
        <li role="none">
          <DropdownItem as="button" eventKey="page-height" trailingIcon={<Shortcut keys={shortcuts['fit-to-height']} />}>
            {t('fit_to_height')}
          </DropdownItem>
        </li>
