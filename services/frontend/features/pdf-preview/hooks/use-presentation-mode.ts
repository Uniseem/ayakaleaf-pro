...
export default function usePresentationMode(
  ...
  const requestPresentationMode = useCallback(() => {
    if (pdfJsWrapper) {
      pdfJsWrapper.container.parentElement?.requestFullscreen().catch(debugConsole.error)
    }
  }, [pdfJsWrapper])
  ...
  const handleFullscreenChange = useCallback(() => {
    if (pdfJsWrapper) {
      const fullscreen = document.fullscreenElement === pdfJsWrapper.container.parentNode
