...
import browser from '@/features/source-editor/utils/browser'
import { debugConsole } from '@/lib/debug'
import type { PDFFile, PdfScrollPosition } from './types'

const DEFAULT_RANGE_CHUNK_SIZE = 128 * 1024 // 128K chunks

/** pdf.js's own viewer, set up the way the preview needs it. */
export default class PDFJSWrapper {
  ...
  async loadDocument({
    ...
    if (pdfFile.size && pdfFile.size < 2 * DEFAULT_RANGE_CHUNK_SIZE) {
      // a small file is fetched in one go
      rangeChunkSize = pdfFile.size
    }

    try {
      const task = loadPdfDocumentFromUrl(url, { rangeChunkSize })
      const abort = () => {
        task.destroy().catch(debugConsole.error)
      }
      ...
      const viewerEl = this.container.querySelector('.pdfViewer') as HTMLElement | null
      const currentHeight = viewerEl?.getBoundingClientRect().height ?? 0
      if (viewerEl && currentHeight > 0) {
      ...
      if (typeof container.startViewTransition === 'function' && document.visibilityState !== 'hidden') {
  ...
  clickPosition(event: MouseEvent, canvas: HTMLCanvasElement | null, page: number) {
    if (!canvas) {
      return
    }
  ...
  get currentPosition(): PdfScrollPosition {
  ...
  scrollToPosition(position: PdfScrollPosition, scale: string | null = null) {
    const destArray = [
      null,
      {
    ...
    // Snap back to 0 so the margin is visible, but only when in that margin band --
    // for any real mid-document scrollTop this condition is false and it is left untouched.
