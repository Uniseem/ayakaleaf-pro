/**
 * Reading what BibTeX or Biber said.
 *
 * BibTeX writes prose ("Warning--... --line 12 of file refs.bib"); Biber
 * writes one bracketed line per event. The first line says which, and each
...
const parserReducer = function (maxErrors: number | null) {
  return function <T>(accumulator: [T[], string], parser: Parser<T>): [T[], string] {
...
export type BibLogResult = {
  all: BibLogEntry[]
  errors: BibLogEntry[]
  warnings: BibLogEntry[]
  files: BibLogEntry[]
  typesetting: BibLogEntry[]
}
...
export default class BibLogParser {
  ...
  constructor(text: string, options: { maxErrors?: number } = {}) {
    ...
    // describes how to transform the regex match into a log entry object.
    this.warningParsers = [
      [
  ...
  parseBibtex(): BibLogResult {
    // reduce over the parsers, starting with the log text,
    const [allWarnings, remainingText] = this.warningParsers.reduce(parserReducer(this.options.maxErrors ?? null), [
      [] as BibLogEntry[],
      this.text,
    ])
    const [allErrors] = this.errorParsers.reduce(parserReducer(this.options.maxErrors ?? null), [
      [] as BibLogEntry[],
      remainingText,
    ])
  ...
  parseBiber(): BibLogResult {
    const result: BibLogResult = {
      all: [],
      errors: [],
      warnings: [],
      files: [], // not used
    ...
    this.lines.forEach(function (line) {
      ...
      if (match) {
        ...
        const newEntry: BibLogEntry = {
          file: '',
          level: MESSAGE_LEVELS[messageType as keyof typeof MESSAGE_LEVELS] || 'INFO',
          message: message ?? '',
          line: '',
          raw: fullLine,
        }
