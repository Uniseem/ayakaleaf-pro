/**
 * What to tell somebody about each kind of compiler message.
 */
import {
...
// Whether the help pages exist on this site: a hint only links to one when
// they do. The original read this from page metadata; here the IDE sets it
// from the site settings once they are known.
let wikiEnabled = true
const originalExtraInfoURLs = new Map(
  Object.entries(hints).map(([ruleId, hint]) => [ruleId, hint.extraInfoURL])
)

export function setWikiEnabled(flag: boolean) {
  if (flag === wikiEnabled) return
  wikiEnabled = flag
  Object.values(hints).forEach((hint, index) => {
    const ruleId = Object.keys(hints)[index] ?? ''
    hint.extraInfoURL = flag ? (originalExtraInfoURLs.get(ruleId) ?? null) : null
  })
}

export function hintFor(ruleId: string) {
  return hints[ruleId]
}
